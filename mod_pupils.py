...
def create_pupil():
    ...
    while fl1:  # it is for valid values
        ...
        name = input("Name: ")
        while name == "":
            name = input("Wrong Input! Name: ")
        name = name.strip()
        name = name.split()
        for i in range(len(name)):
            if not name[i].isalpha():
                fl1 = True  # to maintain the loop
                name = name.clear()
                tempname = ""
    ...
    while fl1:
        ...
        while surname == "":
            surname = input("Wrong Input! Surname: ")
    ...
    while not fathersname.isalpha():
        fathersname = input("Wrong Input! Fathersname: ")
        fathersname = fathersname.strip()
    ...
    if fl11:  # if there are 2 or more persons with name, surname and fathersname appears them
        ...
        while not flag.isdigit() or (flag != "1" and flag != "0"):
            flag = input("Wrong Input! Do you want to continue the registration? If yes press 1 or if no press 0: ")
    if flag == "1":
        age = input("Age: ")
        age = age.strip()
        while not age.isdigit():
            age = input("Wrong Input! Age: ")
        age = age.strip()
        students[y].append(age)
        classroom = input("Class: ")
        classroom = classroom.strip()
        while not classroom.isdigit() or (int(classroom) < 1 or int(classroom) > 6):
            classroom = input("Wrong Input! Class: ")
        classroom = classroom.strip()
    ...
    for i in range(len(students)):
        if students[i][b] == a:
            ...
            if ch == 1:
                ...
                print("\nFathersmane updated successfully!")
            elif ch == 4:
                age = input("Age: ")
                age = age.strip()
                while not age.isdigit():
...
def next_id():  # in case of a pupil is deleted, the new pupil will take the previous pupil's id, otherwise the new
    # pupil takes the next id of the last registered pupil
    for i in range(1, len(students)):
        if students[i][0] != str(i + 1000 - 1):
            return i + 1000 - 1
    else:
